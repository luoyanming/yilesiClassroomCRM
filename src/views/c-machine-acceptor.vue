    ...
    <div class="main-wrapper light-overscroll luoym">
        <section class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>智慧接收器</el-breadcrumb-item>
            </el-breadcrumb>
        </section>
        ...
        <section class="search clearfix">
            ...
            <div class="button-blank">
                ...
                <el-button type="primary" size="small" class="btn-add" icon="plus" @click.native="handleAdd" v-if="role == 2">导入新接收器</el-button>
            </div>
        </section>
        ...
        <section class="table">
            ...
            <el-table :data="tableData" stripe tooltip-effect="dark" style="width: 100%" v-loading="tableloading">
                ...
                <el-table-column label="所处位置">
                    <template scope="scope">{{ scope.row.location }}</template>
                </el-table-column>
        ...
        <el-dialog title="导入新接收器" :visible.sync="addDialogShow" :modal-append-to-body="false">
            <section class="formation">
                ...
                <el-form label-position="right" :rules="addRules" ref="addRuleForm" label-width="180px" :model="addDialogInfo">
                    ...
                    <el-form-item label="接收器版本" prop="version">
        ...
        <el-dialog title="编辑接收器信息" :visible.sync="editDialogShow" :modal-append-to-body="false" class="editDialog-wrapper">
            <section class="formation">
               
                <el-form label-position="right" :rules="editRules" ref="editRuleForm" label-width="180px" :model="editDialogInfo">
                    <div class="flex-h">
                        <div class="flex-a-i">
                            <div style="font-size: 14px; line-height: 2; color: #333; padding: 10px 0 20px 0;">智慧接收器基本信息</div>
                            <div class="">
                        ...
                        <div class="flex-a-i">
                            <div style="font-size: 14px; line-height: 2; color: #333; padding: 10px 0 20px 0;">智慧接收器应用信息</div>
                            <div class="">
                                ...
                                <el-form-item label="所处位置" prop="location">
                                    <el-input v-model="editDialogInfo.location"></el-input>
                                </el-form-item>
                                <el-form-item label="布设方向">
                                    <el-select v-model="editDialogInfo.layDirection" placeholder="请选择">
                                        <el-option v-for="item in layDirectionOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="信号过滤">
                                    <el-select v-model="editDialogInfo.signalFilter" placeholder="请选择">
                                        <el-option v-for="item in signalFilterOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                                    </el-select>
                                </el-form-item>
                            </div>
                        </div>
        ...
        <el-dialog title="导入接收器应用信息" :visible.sync="applicationDialogShow" :modal-append-to-body="false">
            <section class="formation">
...
<script>
    ...
    import { uploadPath, channelList, getAllSmartDeviceVersion, getSchoolListByChannel, smartAcceptorList, smartAcceptorSave, getClassListBySchool, getRegionListBySchool } from '../api/api';
        ...
        data() {
            return {
                ...
                editRules: {
                    ...
                    location: [
                        { required: true, message: '*请输入所处位置', trigger: 'change' }
                    ],
                },
                ...
                // 激活新设备
                uploadLoading: false,
                uploadUrl: uploadPath + '/ajax/acceptor/import/activate',
                fileList: [],
                ...
                uploadApplicationUrl: uploadPath + '/ajax/acceptor/import/applicationInfo',
                applicationFileList: [],
        ...
        methods: {
            ...
            getList: function() {
                ...
                let param = {
                    ...
                    'localtion': this.searchForm.type == 2 ? this.searchForm.typeValue : '',
                ...
                smartAcceptorList(param).then(res => {
            ...
            handleEdit: function(index, row) {
                ...
                setTimeout(function() {
                    ...
                    that.editDialogInfo.location = row.location;
                    that.editDialogInfo.layDirection = '' + row.layDirection;
                    that.editDialogInfo.signalFilter = '' + row.signalFilter;
                ...
                this.$refs[formName].validate((valid)=>{
                    if(valid){
                        ...
                        let params = {
                            ...
                            'location': this.editDialogInfo.location,
                            'layDirection': this.editDialogInfo.layDirection,
                            'signalFilter': this.editDialogInfo.signalFilter
                        };

                        smartAcceptorSave(params).then(res=>{
                            let { errorInfo, code, data } = res;
            ...
            handleDownloadActivate: function() {
                location.href = uploadPath + '/ajax/acceptor/import/template?type=1';
            },   
            // 下载录入模板
            handleDownloadApplication: function() {
                location.href = uploadPath + '/ajax/acceptor/import/template?type=2';
            },
            ...
            handleExportSubmit: function() {
                ...
                location.href = uploadPath + '/ajax/acceptor/export?exportContent=' + exportContent + '&versionIds=' + versionIds + '&channelIds=' + channelIds + '&saleType=' + saleType + '&status=' + status + '&schoolIds=' + schoolIds;
